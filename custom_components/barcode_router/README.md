...
1. Copy the `barcode_router` folder to your Home Assistant `custom_components` directory:
   ```
   <config>/custom_components/barcode_router/
   ```
...
4. Search for "Barcode Router" and follow the setup wizard:
...
The integration is configured through the Home Assistant UI during setup. You can also manually configure it in `configuration.yaml`:

```yaml
barcode_router:
  grocy_url: "http://grocy.local:9283"
